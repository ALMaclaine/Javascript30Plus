/*
    ...
    https://github.com/hugoruivo/randombookjsongenerator
 */
...
const pad = (val) => val.padStart(2).replace(/ /gi, '0');
...
function generateVideoTime() {
    const mins = generateNumberBetween(0, 9);
    const seconds = generateNumberBetween(0, 59);

    return formatTime(0, mins, seconds);
}

function CreateVideo() {
    const li = document.createElement('li');
    const time = generateVideoTime();
    li.dataset.time = time;

    const p = document.createElement('p');
    p.innerText = `${RandomTitle()}: `;
    p.classList.add('video__title');
...
function generateRandomVideos() {
    const numVideos = generateNumberBetween(10, 70);
...
function calculateTotalTime() {
    ...
    let seconds = timeNodes
        .reduce((total, curEl) => {
            const data = curEl.dataset.time;
            const [mins, secs] = data.split(':').map(parseFloat);
            total += (mins * 60) + secs;
            return total;
        }, 0);
    ...
    return formatTime(hours, mins, secondsLeft);
}
...
function addTotalTime() {
    ...
    pTag.innerText = 'Total Time: ';
    pTag.style.display = 'inline';
    const bTag = document.createElement('b');
    bTag.innerText = calculateTotalTime();
...
function removeTotalTime() {
    document.getElementById('totalTime').remove();
}

setTimeout(() => setInterval(intervalCallback, 100), 5000);
